...
#SBATCH --output=/vast/eande106/projects/Lance/THESIS_WORK/pathogen_unalignedBAM_SDSU/processed_data/c_elegans/SLURM_output/initial_krakenBracken_run/EukPathRun.oe  
#SBATCH --error=/vast/eande106/projects/Lance/THESIS_WORK/pathogen_unalignedBAM_SDSU/processed_data/c_elegans/SLURM_output/initial_krakenBracken_run/EukPathRun.rr 
...
# mkdir -p "$raw_data/databases/EukPath"

# if [ ! -s "$raw_data/databases/EukPath/taxo.k2d" ]; then
#     if ! wget --quiet --no-clobber -O "$raw_data/databases/EukPath/k2_eupathdb48_20230407.tar.gz" https://genome-idx.s3.amazonaws.com/kraken/k2_eupathdb48_20230407.tar.gz; then
#         echo "Error downloading database"
#         exit 1
#     fi
#     if ! tar --extract --verbose --file="$raw_data/databases/EukPath/k2_eupathdb48_20230407.tar.gz" --directory="$raw_data/databases/EukPath"; then
